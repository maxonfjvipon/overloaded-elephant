...
class TestOverloadable extends TestCase
{
    ...
    public function testOverloading()
    {
        ...
        self::assertEquals([1, 2.0], $this->overload([1, 2.0], [['integer', 'float']]));
        self::assertEquals([1, 22], $this->overload([1, 22], [
            ['integer'],
        ]));
        self::assertEquals(
            [false, 2, 4.6, 'hello world', [1, 2], "Hello world"],
            ...
            ], 3 => ['string' => fn(string $str) => $str . ' world'], ['array'], [
                Fakeable::class => fn(Fakeable $fk) => $fk->fake()
            ]])
        );
    }
